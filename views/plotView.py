...
class MultiChannelPlotWidget(QWidget):
    ...
    def __init__(self, num_channels=32, visible_duration_seconds=10.0, y_scale=2000.0):
        ...
        # x-axis line at the bottom, with moving time tick labels.
        self.x_axis_line = scene.Line(
            pos=np.array(
                [[0.0, -self.y_scale], [self.visible_duration_seconds, -self.y_scale]],
                dtype=float,
    ...
    def set_signal_time(self, signal_time_seconds):
        """Receive the current signal time from the ViewModel and move the x-axis ticks."""
        self.current_signal_time = float(signal_time_seconds)
        self._update_time_ticks()
    ...
    def _update_camera(self):
        offset = self.channel_offset()
        label_space = 0.08 * (2 * self.y_scale)
